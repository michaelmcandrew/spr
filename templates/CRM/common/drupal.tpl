...
{* We want to use the Drupal CKEditor if it exists, if not we will use the CiviCRM packaged version *}
{* using the Drupal API module_exists($module) function *}
   {if ($config->userFramework == 'Drupal' && module_exists('ckeditor'))}
      {* point to Drupal version of CKEditor *}
      <script type="text/javascript" src="/main/sites/all/modules/ckeditor/ckeditor/ckeditor.js"></script>
   {else}
      {* Not using Drupal or no ckeditor module installed - point to CiviCRM packaged version of CKEditor *}
